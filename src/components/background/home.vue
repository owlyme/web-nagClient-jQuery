  ...
  <el-row :gutter="20">
    <el-col :span="24">
      <el-card shadow="hover" class="mgb20">
        ...
        <div class="flex header-top">
          <div class="h-l flex">
            ...
            <div class="l-name"><img src="@/assets/img/avatar.jpg"></div>
            <div class="flex flexcolumn flexbox infobox flexjustifybetween">
              <div class="tb-margin">
                <span style="font-size:20px">{{!info.lastName?info.email:info.displayName}}</span>
              </div>
              <div class="flex">
                <el-tag v-if="isdemo" d type="info">Demo</el-tag>
                <router-link v-if="isdemo" :to="'/Information?id='+info.id">
                  <el-tag>{{$t('home.createdlive')}}</el-tag>
                </router-link>
                <router-link v-show="!isdemo && !info.approved " :to="'/Information?id='+info.id">
                  <el-tag>{{$t('home.certification')}}</el-tag>
                </router-link>

                <router-link v-show="info.idUploaded == 'AUDIT_APPROVE_REJECTED'" :to="'/Information?id='+info.id">
                ...
                <div v-if="!isdemo">
                  <el-tag v-if="info.approved" type="success">{{info.userWholeStatusDisplay}}</el-tag>
                  <el-tag class="lr-margin" v-if="!info.approved" type="warning">{{info.userWholeStatusDisplay}}</el-tag>
                  <!-- <el-tag
                                            v-if="info.idUploaded == 'AUDIT_NOT_UPLOAD'"
                                            type="danger"
                                        >{{info.approvedDisplay}}</el-tag> -->
          ...
          <div class="flex" v-show="!isdemo" style="padding-top: 10px">
            <div @click="opengo" class="shortcut flex flexcolumn" v-show="!businessshow && info.approved ">
              <img src="@/assets/img/t7.png" alt />
              <div>{{$t('home.opensw')}}</div>
            </div>

            <router-link v-show="businessshow && info.approved" to="Toactivate" class="shortcut flex flexcolumn" tag="div">
              <!-- <i class="el-icon-lx-people "></i> -->
              <img src="@/assets/img/t1.png" alt />
              <div>{{$t('home.Toactivate')}}</div>
            </router-link>
            <router-link v-show="businessshow && info.approved" to="Activated" class="shortcut flex flexcolumn" tag="div">
            ...
            <router-link to="upload" class="shortcut flex flexcolumn" tag="div" v-show="info.idUploaded != 'AUDIT_NOT_UPLOAD'">
      ...
      <!-- <el-card
                ...
                > -->
      <el-card shadow="hover" v-show="!businessshow ||
        ...
        <div slot="header" class="clearfix">
          ...
          <p v-show="!isdemo && !info.approved">
            {{$t('home.hint1')}}
            <router-link class="text-color" tag="a" :to="'/Information?id='+info.id">{{$t('home.hint2')}}</router-link>{{$t('home.hint3')}}
          </p>
          <p v-show="isdemo">
            {{$t('home.hint4')}}
            <router-link class="text-color" tag="a" :to="'/Information?id='+info.id">{{$t('home.hint5')}}</router-link>，{{$t('home.hint6')}}
          </p>
          <p v-show="!isdemo && info.idUploaded == 'AUDIT_UPLOADED'">{{$t('home.hint7')}}</p>
        </div>
        <el-row :gutter="0" class="mgb20">
          ...
          <el-col :span="8" class="text-main">
            <div class="text-conter">
              ...
              <p class="text">
                <span class="strong">+</span>{{$t('home.hint17')}}
              </p>
              <p class="text">
                <span class="strong">+</span> {{$t('home.hint18')}}
              </p>
              <p class="text">
                <span class="strong">+</span>{{$t('home.hint19')}}
              </p>
              <p class="text">
                <span class="strong">+</span> {{$t('home.hint20')}}
              </p>
            </div>
      ...
      <el-card shadow="hover" v-show="info.approved || isdemo " class="table-container">
        <el-table :data="tableData" border style="width: 100%" stripe>
          <el-table-column align="left" prop="tradeLoginId" :label="$t('message.tradingaccount')"></el-table-column>
          ...
          <el-table-column align="left" prop="typeDisplay" :label="$t('message.accounttype')"></el-table-column>
          <el-table-column align="left" :label="$t('message.Accountcurrency')">
            <template slot-scope="scope">USD</template>
          </el-table-column>
          <el-table-column align="left" prop="info.balance" :label="$t('message.balance')"></el-table-column>
          <el-table-column align="left" prop="info.floating" :label="$t('message.floatingprofitandloss')"></el-table-column>
          <el-table-column align="left" prop="info.credit" :label="$t('message.CreditLimit')"></el-table-column>
          <el-table-column align="left" prop="info.equity" :label="$t('message.networth')"></el-table-column>
          <el-table-column align="left" prop="info.marginfree" :label="$t('message.UsableMargin')"></el-table-column>
          <el-table-column min-width="120" align="left" prop="info.marginlevel" :label="$t('message.UsableMargin')+'%'"></el-table-column>
          <el-table-column align="left" :resizable="false" fixed="right" :label="$t('message.operation')" min-width="130">
            <template slot-scope="scope">
              <el-button style="color: #00a3db;" type="text" @click="changegg(scope.row.tradeLoginId)" size="small">{{$t('home.hint10')}}</el-button>
              <el-button style="color: #00a3db;" type="text" size="small" @click="changepassword(scope.row.tradeLoginId)">{{$t('title.changepassword')}}</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
    ...
    <el-dialog :title="$t('home.hint11')" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
    ...
    <el-dialog :title="$t('message.changetrade')" :visible.sync="showloginalter" width="30%" :before-close="handleClose">
      <el-form ref="form" :model="changedata" label-width="120px">
        <el-form-item :label="$t('message.accounttype')">
          <el-select class="fullwidth" v-model="changedata.type" :placeholder="$t('message.accounttype')">
            <el-option v-for="(e,i) in actions" :label="e.name" :value="e.value"></el-option>
        ...
        <el-form-item :label="$t('message.Repeatnewpassword')">
          <el-input class="fullwidth" show-password v-model="changedata.repeatPassword"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog :title="$t('home.hint10')" :visible.sync="showgg" width="30%" :before-close="handleClose">
      <el-form ref="form" :model="gg" label-width="80px">
        <el-form-item :label="$t('home.hint12')">
          <el-select v-model="gg.leverage" :placeholder="$t('home.hint12')">
            <el-option v-for="(e,i) in otherExplist" :label="e.key" :value="e.value"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  data() {
    ...
    return {
      showalter: false,
      lblist: banner,
      businessshow: localStorage.businessshow ? true : false,
      isdemo: false,
      otherExplist: [
