...
<el-row :gutter="20">
  <el-col :span="24">
    <el-card shadow="hover" class="mgb20">
      ...
      <el-button type="primary" @click="changedata(id)">{{$t('message.save')}}</el-button>
                </div>-->
      <div slot="header" class="clearfix">
        <span>{{$t('message.namemaid')}}:{{params.name}}</span>

        <el-button v-show="type != 'look'" style="float: right;" type="primary" @click="changedata(id)">{{$t('message.save')}}</el-button>
        <el-button class="lr-margin" style="float: right;" type="info" @click="back">{{$t('message.return')}}</el-button>
      </div>
        ...
        <tbody>
          <tr v-for="(item,index) in tablelist" v-if="item.type == 1">
            ...
            placeholder=""
              ...
              <el-input-number @change="onChange(item.profit,index)" v-model="item.profit" :min="0"></el-input-number>
              <!-- {{item.profit}} -->
            </td>
            <td>
              {{item.subAmount}}
      ...
      <table class="tb_box">
        ...
        <tbody>
          <tr v-for="(item,index) in tablelist" v-if="item.type == 2">
            ...
            <td>{{item.amount}}</td>
            <td>
              <el-input-number @change="onChangetow(item.subAmount,index,item.amount)" v-model="item.subAmount" :max="item.amount" :min="0"></el-input-number>
